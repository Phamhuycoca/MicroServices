using AuthService.Application.DTO;
using AuthService.Application.IServices;
using AuthService.Domain.Entities;
using AuthService.Infrastructure.IRepository;
using AutoMapper;
using Microsoft.AspNetCore.Identity;
using Microsoft.EntityFrameworkCore;
using Microsoft.Extensions.Options;
using Microsoft.IdentityModel.Tokens;
using System;
using System.Collections.Generic;
using System.IdentityModel.Tokens.Jwt;
using System.Linq;
using System.Security.Claims;
using System.Security.Cryptography;
using System.Text;
using System.Threading.Tasks;
using static Microsoft.EntityFrameworkCore.DbLoggerCategory;

namespace AuthService.Application.Services
{
    public class ServiceRefreshToken : IServiceRefreshToken
    {
        private readonly IRefreshTokenRepo _repo;
        private readonly UserManager<nguoi_dung> _userManager;
        private readonly JwtOptions _jwt;
        private readonly IMapper _mapper;
        public ServiceRefreshToken(
            IOptions<JwtOptions> jwtOptions,
            UserManager<nguoi_dung> userManager,
            IRefreshTokenRepo repo,
            IMapper _mapper)
        {
            _userManager = userManager;
            _repo = repo;
            _jwt = jwtOptions.Value;
            _mapper = _mapper;
        }

        public async Task<TokenResponseDto> GenerateTokensAsync(nguoi_dung user, string ipAddress)
        {
            try
            {
                var accessTokenExpires = DateTime.UtcNow
           .AddMinutes(_jwt.AccessTokenMinutes);

                var accessToken = await GenerateAccessToken(user, accessTokenExpires);

                // 2. Refresh Token
                var refreshToken = GenerateRefreshToken();

                // 3. Lưu DB
                var entity = new refresh_token
                {
                    Id = Guid.NewGuid(),
                    Token = refreshToken,
                    nguoi_dung_id = user.Id,
                    ExpiresAt = DateTime.UtcNow.AddDays(_jwt.RefreshTokenDays),
                    CreatedAt = DateTime.UtcNow,
                    CreatedByIp = ipAddress,
                    IsRevoked = false
                };

                 _repo.Add(entity);

                return new TokenResponseDto
                {
                    AccessToken = accessToken,
                    RefreshToken = refreshToken,
                    AccessTokenExpiresAt = accessTokenExpires
                };
            }
            catch (Exception ex)
            {
                throw;
            }
        }

        public Task<RefreshTokenDTO> GetByTokenAsync(RefreshTokenRequest refreshToken)
        {
            try
            {
                var entity = _repo.GetAll().Where(x => x.Token == refreshToken.RefreshToken).FirstOrDefault();
                if(entity == null)
                    return Task.FromResult<RefreshTokenDTO>(null);
                return Task.FromResult(new RefreshTokenDTO
                {
                    Id = entity.Id,
                    Token =  entity.Token,
                    nguoi_dung_id = entity.nguoi_dung_id,
                    ExpiresAt = entity.ExpiresAt,
                    IsRevoked = entity.IsRevoked,
                    CreatedAt = entity.CreatedAt,
                    CreatedByIp = entity.CreatedByIp
                });
            }
            catch (Exception ex)
            {
                throw;
            }
        }

        public Task<RefreshTokenDTO> UpdateToken(RefreshTokenDTO refreshToken)
        {
            try
            {
                var entity = _repo.GetAll().Where(x => x.Id == refreshToken.Id).FirstOrDefault();
                if (entity == null)
                    throw new Exception("Không tìm thấy refresh token");
                entity.IsRevoked = refreshToken.IsRevoked;
                var dto =_repo.Update(entity);
                entity.IsRevoked = refreshToken.IsRevoked;
                var updated = _repo.Update(entity);
                return Task.FromResult(_mapper.Map<RefreshTokenDTO>(updated));
            }
            catch (Exception ex)
            {
                throw;
            }
        }

        private async Task<string> GenerateAccessToken(
        nguoi_dung user,
        DateTime expires)
        {
            var claims = new List<Claim>
        {
            new Claim(JwtRegisteredClaimNames.Sub, user.Id.ToString()),
            new Claim(JwtRegisteredClaimNames.UniqueName, user.UserName),
            new Claim(JwtRegisteredClaimNames.Jti, Guid.NewGuid().ToString())
        };

            var roles = await _userManager.GetRolesAsync(user);
            claims.AddRange(roles.Select(r => new Claim(ClaimTypes.Role, r)));

            var key = new SymmetricSecurityKey(
                Encoding.UTF8.GetBytes(_jwt.SecretKey)
            );

            var creds = new SigningCredentials(key, SecurityAlgorithms.HmacSha256);

            var token = new JwtSecurityToken(
                issuer: _jwt.Issuer,
                audience: _jwt.Audience,
                claims: claims,
                expires: expires,
                signingCredentials: creds
            );

            return new JwtSecurityTokenHandler().WriteToken(token);
        }

        private string GenerateRefreshToken()
        {
            return Convert.ToBase64String(RandomNumberGenerator.GetBytes(64));
        }
    }
}
